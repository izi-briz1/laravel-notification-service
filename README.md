...
`php artisan test` - 29 интеграционных и unit-тестов (152 assertions) против реальных PostgreSQL/Redis: сквозной тест всей цепочки (HTTP-запрос -> очередь -> job -> провайдер -> статусы и история в БД -> ответ API подписчика), создание рассылки и маршрутизация по приоритетам, идемпотентность (включая потерю ключа Redis), цепочка job -> провайдер -> статус в БД, exactly-once при redelivery, transient/permanent-ошибки и исчерпание попыток, rate limit канала, outbox-свипер, dead-worker-свипер для зависших `sending`, webhook, история статусов, фильтры и пагинация.
