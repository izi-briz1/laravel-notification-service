...
Приоритизация трафика: Система должна гарантировать доставку критичных сообщений без задержек. Транзакционные уведомления (коды доступа, срочные изменения маршрута) должны получать наивысший приоритет и отправляться вне очереди, обгоняя маркетинговые рассылки.
...
Ретри-механизмы: Автоматический повтор попыток отправки при временной недоступности шлюзов.
...
Весь проект (БД, брокер, приложение) должен запускаться одной командой docker-compose up.
...
Опубликуй код в публичный репозиторий на GitLab или GitHub и пришли ссылку.
В файле README.md опиши пошаговую инструкцию по запуску через docker-compose.
Предоставь описание API для тестирования. Ссылка на Swagger (OpenAPI) или приложенная Postman-коллекция будет идеальным решением.

Удачи! Мы ценим основательный подход к архитектуре и качеству кода. Ждем твое решение!
